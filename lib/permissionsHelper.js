...
var propagatePermissions = function(couchbase, dbs, msg, perms){
   ...
   for ( var type_id in type_updates_all ){
      ...
      updateObjectPermissionsByType(couchbase, dbs, cloudlet_id, type_id, thirdPartyId, third_party_cloudlet, permissions, objects, false)
   }

   for ( var type_id in type_updates_app ){
      var permissions = type_updates_app[type_id]
      updateObjectPermissionsByType(couchbase, dbs, cloudlet_id, type_id, thirdPartyId, third_party_cloudlet, permissions, objects, true)
   }
   ...
   for ( var object_id in object_updates ){
      ...
      funs[i] = function(dbs, cloudlet_id, object_id, thirdPartyId, third_party_cloudlet, permissions, applevel){
         return function (callback) {
            updateObjectsPermission(dbs, cloudlet_id, object_id, thirdPartyId, third_party_cloudlet, permissions, applevel, callback)
         }
      }(dbs, cloudlet_id, object_id, thirdPartyId, third_party_cloudlet, permissions, false)
   }
...
var updateObjectPermissionsByType = function(couchbase, dbs, cloudlet_id, type_id, thirdPartyId, thirdPartyCloudlet, permissions, objects, applevel){
   ...
   dbs['objects'].query(query, function (err, res) {
      ...
      for (var i = 0; i < res.length; i ++){
         ...
         if ( -1 === objects.indexOf(object_id) ){
            funs[i] = (function(dbs, cloudlet_id, object_id, thirdPartyId, thirdPartyCloudlet, permissions, applevel){
               return function (callback) {
                  updateObjectsPermission(dbs, cloudlet_id, object_id, thirdPartyId, thirdPartyCloudlet, permissions, applevel, callback)
               }
            }(dbs, cloudlet_id, object_id, thirdPartyId, thirdPartyCloudlet, permissions, applevel))
         }
      }
...
var updateObjectsPermission = function(dbs, cloudlet_id, object_id, thirdPartyId, third_party_cloudlet, permissions, applevel, callback){
   ...
   dbs['objects'].get(key, function (err, db_body) {
      ...
      if ( !applevel || (applevel && db_body.value._permissions.created_by_app === thirdPartyId) ){
         ...
         db_body.value._permissions[third_party_cloudlet] = permissions
