...
var init = function (logger_params, tracklet_worker) {
...
var checkPermissions = function(msg, callback, success_function){
   ...
   if (msg.third_party === msg.cloudlet_id){
      var permissions = {}
      permissions['created_by']     = third_party
      success_function(permissions)
   }
   else if (openiUtils.isTypeId(msg.database) || openiUtils.isAttachmentId(openiUtils.extractAttachmentId(msg.database))){
      success_function()
   }
   else{
...
var processGetData = function (db_body, msg, callback) {
   ...
   default:
         trackletWorker.send({
            ...
            type_id              : db_body.value["@openi_type"],
            headers              : msg.headers
         })
